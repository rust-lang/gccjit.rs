...
fn main() {
    ...
    let parameter = context.new_parameter(None, int_ty, "n").unwrap();
    let factorial = context.new_function(None,
                                         ...
                                         "factorial",
                                         false).unwrap();
    let block = factorial.new_block("entry").unwrap();
    let parm = factorial.get_param(0).unwrap().to_rvalue();
    // if (n == 0) goto recurse_block else goto ret_block
    let cond = context.new_comparison(None,
                                      ComparisonOp::Equals,
                                      parm,
                                      context.new_rvalue_zero(int_ty).unwrap()).unwrap();
    let false_branch = factorial.new_block("recurse_block").unwrap();
    let true_branch = factorial.new_block("ret_block").unwrap();
    block.end_with_conditional(None, cond, true_branch, false_branch);
    // ret_block: return 1
    true_branch.end_with_return(None, context.new_rvalue_one(int_ty).unwrap());
    // recurse_block: return n * fact(n-1)
    let n_minus_one = parm - context.new_rvalue_one(int_ty).unwrap();
    let call = context.new_call(None, factorial, &[n_minus_one]).unwrap();
    let mul = parm * call;
