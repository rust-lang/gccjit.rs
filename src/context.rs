...
use block::{self, BinaryOp, Block, ComparisonOp, UnaryOp};
...
impl<'ctx> Context<'ctx> {
    ...
    pub fn set_program_name<S: AsRef<str>>(&self, name: S) {
        ...
        unsafe {
            gccjit_sys::gcc_jit_context_set_str_option(
                self.ptr,
    ...
    pub fn add_command_line_option<S: AsRef<str>>(&self, name: S) {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe { gccjit_sys::gcc_jit_context_add_command_line_option(self.ptr, c_str.as_ptr()) }
    }

    pub fn add_driver_option<S: AsRef<str>>(&self, name: S) {
        let c_str = CString::new(name.as_ref()).unwrap();
        unsafe { gccjit_sys::gcc_jit_context_add_driver_option(self.ptr, c_str.as_ptr()) }
    }
    ...
    pub fn set_optimization_level(&self, level: OptimizationLevel) {
        unsafe {
            gccjit_sys::gcc_jit_context_set_int_option(
                self.ptr,
    ...
    pub fn new_case<S: ToRValue<'ctx>, T: ToRValue<'ctx>>(
        ...
        let result = unsafe {
            Case {
                marker: PhantomData,
                ptr: gccjit_sys::gcc_jit_context_new_case(
                    self.ptr,
    ...
    pub fn new_location<'a, S: AsRef<str>>(
        &'a self,
        filename: S,
        line: i32,
        col: i32,
    ...
    pub fn new_field<'a, S: AsRef<str>>(
        ...
        unsafe {
            ...
            let ptr = gccjit_sys::gcc_jit_context_new_field(
                self.ptr,
                loc_ptr,
                types::get_ptr(&ty),
                cstr.as_ptr(),
    ...
    pub fn new_call<'a>(
        ...
        let mut params_ptrs: Vec<_> = args.iter().map(|x| unsafe { rvalue::get_ptr(x) }).collect();
        unsafe {
    ...
    pub fn new_rvalue_vector_perm<'a>(
        ...
        unsafe {
            ...
            let ptr = gccjit_sys::gcc_jit_context_new_rvalue_vector_perm(
                self.ptr,
                loc_ptr,
    ...
    pub fn new_struct_constructor<'a>(
        ...
        unsafe {
            ...
            let fields_ptr = match fields {
                Some(fields) => fields.as_ptr(),
                None => ptr::null_mut(),
            };

            let ptr = gccjit_sys::gcc_jit_context_new_struct_constructor(
                self.ptr,
                loc_ptr,
    ...
    pub fn new_array_constructor<'a>(
        ...
        unsafe {
            ...
            let ptr = gccjit_sys::gcc_jit_context_new_array_constructor(
                self.ptr,
                loc_ptr,
    ...
    pub fn new_vector_access<'a>(
        ...
        unsafe {
            ...
            let ptr = gccjit_sys::gcc_jit_context_new_vector_access(
                self.ptr,
                loc_ptr,
    ...
    pub fn dump_to_file<S: AsRef<str>>(&self, path: S, update_locations: bool) {
        unsafe {
            let path_ref = path.as_ref();
            let cstr = CString::new(path_ref).unwrap();
            gccjit_sys::gcc_jit_context_dump_to_file(
                self.ptr,
                cstr.as_ptr(),
    ...
    pub fn get_target_builtin_function<'a, S: AsRef<str>>(&'a self, name: S) -> Function<'a> {
        ...
        unsafe {
            let cstr = CString::new(name_ref).unwrap();
            let ptr =
                gccjit_sys::gcc_jit_context_get_target_builtin_function(self.ptr, cstr.as_ptr());
            #[cfg(debug_assertions)]
            if let Ok(Some(error)) = self.get_last_error() {
                panic!("{}", error);
            }
